...
intOperators[ binaryen['MulInt32'] ] = true;
...
var visitBlockId = function(walker, funcItem, parentNodes, expr) {
	...
	if ( true === output['optimize_for_js'] && 0 !== expr.children.length && '' !== expr.name )
	{
		(function x(children) {
			while (0 !== children.length) {
				const firstExpr = binaryen.getExpressionInfo(children[0]);
				if ( binaryen['GlobalSetId'] === firstExpr.id || binaryen['LocalSetId'] === firstExpr.id ) {
					...
					children.splice(0, 1,
						decodedModule.block( firstExpr.name, firstExpr.children, firstExpr.type )
					);
				}
				break;
			}
		...
		const optimizedResult = (function blockOptimizer(blockType, blockName, children) {
			if ( '' === blockName ) { return children; }
			for ( let i = children.length-1 ; i !== -1 ; --i ) {
				const firstExpr = binaryen.getExpressionInfo(children[i]);
				if ( binaryen['BlockId'] === firstExpr.id ) {
					const retVal = blockOptimizer(firstExpr.type, firstExpr.name, firstExpr.children);
			...
			}
			{
				...
				const isLastChild = function(item, idx, array) {
					...
					return parentArr[parentArr.length-1] === item['srcPtr'] ? true : false;
				};
				...
				const compileWasmExpressions = function(arr) {
					while (0 !== arr.length) {
						...
						const parentInst = (
							0 === arr.length ? null : arr[arr.length-1]
						);
						const createdExpr = (
							binaryen['IfId'] === popped['id'] ?
							decodedModule.if( popped['condition'], popped['ifTrue'], popped['ifFalse'] ) :
							decodedModule.block( popped['name'], popped['children'], popped['type'] )
						)
						if ( null === parentInst || binaryen['BlockId'] === parentInst['id'] ) {
							const parentArr = (
								null === parentInst ? children : parentInst['children']
							);
							const id = parentArr.indexOf(
								popped['srcPtr']
							);
				...
				for ( let i = b.length-1 ; i !== -1 ; --i ) {
					...
					if ( 2 <= curr['stack'].length ) {
						...
						if ( curr['stack'].slice(0, -2).every(isLastChild) ) {
							if ( binaryen['IfId'] === j[0].id && 0 === j[0].ifFalse && 0 !== j[0].ifTrue ) {
								if ( j[1]['srcPtr'] === j[0].ifTrue && binaryen['BlockId'] === j[1].id &&
									curr['srcPtr'] === j[1].children[j[1].children.length-1] ) {
									const parentInst = (
										2 === curr['stack'].length ?
											null : curr['stack'].slice(-3)[0]
									);
									...
									parentArr.splice(id, parentArr.length-id,
										decodedModule.if(
											j[0].condition,
											decodedModule.block('', j[1].children.slice(0, -1)),
											(0 !== ifFalseArr.length ?
												decodedModule.block('', ifFalseArr) : 0
											)
										)
									);
					...
					//TO
					//if (!..) {..}
					if ( binaryen['BreakId'] === curr.id && 0 === curr.value && 0 !== curr.condition ) {
						const parentInst = (
						...
						const isAlternativePattern = (function () {
							const lastChild = binaryen.getExpressionInfo(parentArr[parentArr.length-1]);
							return (binaryen['BreakId'] === lastChild.id && blockName === lastChild.name &&
								0 === lastChild.value && 0 === lastChild.condition);
						})();
						if ( isAlternativePattern || curr['stack'].every(isLastChild) )
						{
							...
							if ( isAlternativePattern ) {
								...
								parentArr.splice(id, parentArr.length-id-1,
									decodedModule.if(
...
var visitIfId = function(walker, funcItem, parentNodes, expr) {
	return new UglifyJS.AST_If({
		...
		alternative: (0 === expr.ifFalse ? null :
...
var visitLocalGetId = function(walker, funcItem, parentNodes, expr, alignType) {
	...
	if ( binaryen['CallId'] === parentNodes[parentNodes.length-1].id )
	{
		...
		if ( -1 !== idx )
			return makeAsmCoercion( node, localType, localType );
	}
	if ( binaryen['BinaryId'] === parentNodes[parentNodes.length-1].id )
	{
		...
		if ( binaryen['f64'] !== expr.type )
		{
			if ( binaryen['DivFloat32'] === parentNode.op ||
...
var visitGlobalGetId = function(walker, funcItem, parentNodes, expr, alignType) {
	const global = prepareAsmJsGlobal(expr.name);
	const node = new UglifyJS.AST_SymbolRef({
		name: ['$','g','_',global['encoded_name']].join('')
	});
...
var visitConstId = function(walker, funcItem, parentNodes, expr, alignType) {
	if ( binaryen['auto'] !== alignType && expr.type !== alignType ) 
		throw ['ConstId: expr.type !== alignType (', expr.type, ' !== ', alignType, ')'].join('');
/*
	Peculiar wasm case:
	(f32.lt
	  (local.get $0)
					...
					start: {
						raw: (
							Math.floor(expr.value) === expr.value ?
								expr.value.toFixed(1) : expr.value.toString(10)
						)
					}
				})
			]
...
var visitBinaryId = function(walker, funcItem, parentNodes, expr, alignType) {
	if ( binaryen['auto'] !== alignType && expr.type !== alignType ) 
		throw ['BinaryId: expr.type !== alignType (',expr.type,' !== ',alignType,')'].join('');
		...
		(binaryen['SubInt32'] === expr.op || binaryen['AddInt32'] === expr.op) )
	{
		...
		if ( binaryen['ConstId'] === right.id &&
			...
			right.value < 0 )
		{
			expr.op = (binaryen['SubInt32'] === expr.op ? binaryen['AddInt32'] : binaryen['SubInt32']);
	...
	const prepareNode = (ptr) => {
		...
		//
		else if ( binaryen['AddFloat32'] <= expr.op && expr.op <= binaryen['MaxFloat32'] )
		{ return walker( expr, ptr, binaryen['f32'] ); }
		else if ( binaryen['EqFloat32'] <= expr.op && expr.op <= binaryen['GeFloat32'] )
		{ return walker( expr, ptr, binaryen['f32'] ); }
		//
		else if ( binaryen['AddFloat64'] <= expr.op && expr.op <= binaryen['MaxFloat64'] )
		{ return walker( expr, ptr, binaryen['f64'] ); }
		else if ( binaryen['EqFloat64'] <= expr.op && expr.op <= binaryen['GeFloat64'] )
		{ return walker( expr, ptr, binaryen['f64'] ); }
		//
		else if ( binaryen['AddInt32'] <= expr.op && expr.op <= binaryen['GeUInt32'] )
		{ return walker( expr, ptr, binaryen['i32'] ); }
...

	{
		...
		return ( true !== annotationList[expr.op] ||
			(binaryen['StoreId'] === parentExpr.id && expr.srcPtr === parentExpr.value) ||
			((binaryen['StoreId'] === parentExpr.id || binaryen['LoadId'] === parentExpr.id) &&
				expr.srcPtr === parentExpr.ptr && 1 !== parentExpr.bytes) ||
			(binaryen['DivSInt32'] !== expr.op && binaryen['DivUInt32'] !== expr.op &&
				(
					binaryen['BinaryId'] === parentExpr.id && binaryen['i32'] === expr.type &&
						!cmpOperators[parentExpr.op]
				)
			)
		) ? node : makeAsmAnnotation(node, expr.type);
	}
};
	...
	const callFnSurname = (
		function () {
			...
			if ( undefined !== callFn['encoded_name'] ) {
				return [
					('' !== callFn['base'] ? 'if' : 'f'), callFn['encoded_name']
				].join('_')
			}
			return callFn['name'];
		}
	...
	res = (
		binaryen['none'] !== expr.type &&
			('undefined' === callFn['stdtype'] || callFn['stdtype'] !== expr.type) ?
				makeAsmAnnotation(res, expr.type) : res
	);

	return (
		binaryen['DropId'] === parentExpr.id || binaryen['BlockId'] === parentExpr.id
	) ? new UglifyJS.AST_SimpleStatement({ body: res }) : res;
};

var visitCallIndirectId = function(walker, funcItem, parentNodes, expr, alignType) {
	const parentExpr = parentNodes[parentNodes.length-1];
	const funcType = [expr.type].concat(
		expr.operands.map( e => (e = binaryen.getExpressionInfo(e), e.type) )
	);
	const funcTypeSignature = funcType.map(
		i => {
			const h = {};
			h[binaryen['none']] = 'v';
			h[binaryen['i32']] = 'i';
	...
	const ftable = prepareFunctionTable(funcItem.idx, funcType[0], funcType.slice(1), funcTypeSignature.join(''));
		...
		expression: new UglifyJS.AST_Sub({
			...
			property: walker(
				...
				decodedModule['i32'].and(
					expr.target, decodedModule['i32'].const(ftable['funcList'].length-1)
				),
			)
		}),
		args: expr.operands.map( e => walker( expr, e ) )
	});
	res = binaryen['none'] !== expr.type ? makeAsmAnnotation(res, expr.type) : res;
...
//-	Sign inversion: type correction required
	if ( binaryen['EqZInt32'] === expr.op )
	{
		if ( true === output['optimize_for_js'] ) {
			...
			if ( !(binaryen['UnaryId'] === exprObj.id && binaryen['EqZInt32'] === exprObj.op) ) {
	...
	if ( binaryen['NegFloat32'] === expr.op || binaryen['NegFloat64'] === expr.op )
	{
		const node = walker( expr, expr.value );
		return new UglifyJS.AST_UnaryPrefix({
	...
	if ( binaryen['FloorFloat32'] === expr.op || binaryen['FloorFloat64'] === expr.op ||
		binaryen['CeilFloat32'] === expr.op || binaryen['CeilFloat64'] === expr.op )
	{
		if ( binaryen['FloorFloat32'] === expr.op || binaryen['CeilFloat32'] === expr.op ) {
			addAsmJsHeader('Math_fround');
		}
		var desiredFunc = '';
		if ( binaryen['FloorFloat32'] === expr.op || binaryen['FloorFloat64'] === expr.op ) {
		...
		return (function () {
			...
			return (
				binaryen['CeilFloat32'] === expr.op || binaryen['FloorFloat32'] === expr.op ?
					new UglifyJS.AST_Call({
						expression: new UglifyJS.AST_SymbolRef({ name: ['$', 'fround'].join('') }), args: [ res ]
					}) : res
			);
		})();
	}
	...
	if ( binaryen['TruncUFloat32ToInt32'] === expr.op || binaryen['TruncUFloat64ToInt32'] === expr.op )
	{
		const fromType = binaryen['TruncUFloat32ToInt32'] === expr.op ? binaryen['f32'] : binaryen['f64'];
		return (
			new UglifyJS.AST_Binary({
				left: makeAsmCoercion( walker( expr, expr.value ), fromType, binaryen['i32'] ),
				operator: '>>>',
				right: new UglifyJS.AST_Number({ value: 0 })
			})
		);
	}
	if ( binaryen['TruncSFloat32ToInt32'] === expr.op || binaryen['TruncSFloat64ToInt32'] === expr.op )
	{
		const fromType = binaryen['TruncSFloat32ToInt32'] === expr.op ? binaryen['f32'] : binaryen['f64'];
		return makeAsmCoercion( walker( expr, expr.value ), fromType, binaryen['i32'] );
	}
	if ( binaryen['PromoteFloat32'] === expr.op )
	{
		return makeAsmCoercion( walker( expr, expr.value ), binaryen['f32'], binaryen['f64'] );
	}
	if ( binaryen['DemoteFloat64'] === expr.op )
	{
		return makeAsmCoercion( walker( expr, expr.value ), binaryen['f64'], binaryen['f32'] );
	}
	if ( binaryen['ConvertSInt32ToFloat32'] === expr.op || binaryen['ConvertUInt32ToFloat32'] === expr.op ||
		binaryen['ConvertSInt32ToFloat64'] === expr.op || binaryen['ConvertUInt32ToFloat64'] === expr.op )
	{
		const fromType = (
			binaryen['ConvertUInt32ToFloat32'] === expr.op ||
			binaryen['ConvertUInt32ToFloat64'] === expr.op ?
			coercionTypes['u32'] : binaryen['i32']
		);
		const destType = (
			binaryen['ConvertSInt32ToFloat32'] === expr.op ||
			binaryen['ConvertUInt32ToFloat32'] === expr.op ?
			binaryen['f32'] : binaryen['f64']
		);
		return makeAsmCoercion( walker( expr, expr.value ), fromType, destType );
	}
	throw [
		'Unary operation, opcode not implemented (', expr.op, ')',
		'\n', (function() {
