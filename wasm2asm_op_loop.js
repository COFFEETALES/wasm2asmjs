...

{
	var visitBreakId = function(walker, funcItem, parentNodes, expr) {
		if ( 0 !== expr.value )
			throw 'BreakId: 0 !== expr.value';
		if ( '' === expr.name )
			throw 'BreakId: \'\' === expr.name';

		const dest = (
			function() {
				const arr = parentNodes.slice().reverse();
				return (
					arr.find( i => binaryen['LoopId'] === i.id && -1 !== i['nameList'].indexOf(expr.name) ) ||
		...
		const node = (
			new AST_T({
				label: (
					function() {
						const parentLoop = parentNodes.filter( i => binaryen['LoopId'] === i.id ).pop();
		...
		$label_1: if ( true === output['optimize_for_js'] ) {
			{
				...
				if ( item ) {
					if ( 0 === item.value ) {
						if ( 0 === item.condition ) {
							...
							resultLoop = (
								new UglifyJS.AST_Do({
									condition: walker( {id: binaryen['LoopId'], name: expr.name}, item.condition ),
									body: new UglifyJS.AST_BlockStatement({
										body: (
											blockExpr.children.pop(),
											walker( [expr, blockExpr],
				...
				const arr = blockExpr.children.slice(-2).map( i => binaryen.getExpressionInfo(i) );
				if ( 2 === arr.length && arr.every( i => binaryen['BreakId'] === i.id && 0 === i.value ) ) {
							...
							if ( 0 === arr[1].condition ) {
								resultLoop = (
									new UglifyJS.AST_For({
										body: new UglifyJS.AST_BlockStatement({
											body: (
												blockExpr.children.splice(-2, 2,
													decodedModule.br(arr[1]['name'], createEqz(arr[0].condition), 0)
												),
												walker( [expr, blockExpr],
													decodedModule.block('', blockExpr.children, blockExpr.type)
												).flat()
				...
				if ( binaryen['IfId'] === lastExpr.id && 0 === lastExpr.ifFalse ) {
					...
					if ( binaryen['BlockId'] === bodyExpr.id ) {
						const item = getUnlinkedBr(blockExpr.children).find(
							elem => bodyExpr.children.pop() === elem['srcPtr'] && expr.name === elem['name']
						);
						if ( item && 0 === item.condition && 0 === item.value ) {

							const bodyArray = (
								Array.prototype.splice.apply(
									blockExpr.children, [-1, 1].concat(bodyExpr.children)
								),
								walker( [expr, blockExpr],
									decodedModule.block('', blockExpr.children, blockExpr.type)
								).flat()
							...
							bodyArray.splice(bodyExpr.children.length * -1, 0,
								new UglifyJS.AST_If({
									condition: walker( [expr, blockExpr], createEqz(lastExpr.condition) ),
		...
		if ( null === resultLoop ) {
			...
			resultLoop = (
				new UglifyJS.AST_While({
					...
					body: new UglifyJS.AST_BlockStatement({
						body: walker(
							[expr, blockExpr], decodedModule.block('', blockExpr.children, blockExpr.type)
						).flat().concat( new UglifyJS.AST_Break({}) )
					})
				})
			);
		}
		...
		if ( true === expr['nested'] ) {
			return (
				new UglifyJS.AST_LabeledStatement({
					label: new UglifyJS.AST_Label({ name: labelValue }), body: resultLoop
				})
			);
		}
		return resultLoop;
