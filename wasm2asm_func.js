...

{
	...
	var makeAsmAnnotation = function(node, resultType) {
		...
		{
			throw ['makeAsmAnnotation: argument type not implemented: ', resultType].join('');
		}
	};
	...
	var makeAsmCoercion = function(node, sourceType, resultType) {
		...
		if ( coercionTypes['f32'] === sourceType )
		{
			...
			if ( coercionTypes['f64'] === resultType )
			{
				return new UglifyJS.AST_UnaryPrefix({ operator: '+', expression: node });
			}
			if ( coercionTypes['f32'] === resultType )
			{
	...
	var asmJsFuncList = (function() {
		for ( let funcIdx = 0 ; -1 !== (funcIdx = wasmFunctions.findIndex( el => null === el['ast'] )) ; )
		{
						...
						new UglifyJS.AST_SimpleStatement({
							body: new UglifyJS.AST_Assign({
								...
								right: makeAsmAnnotation(
									new UglifyJS.AST_SymbolRef({ name: paramName }), arr[i]
								)
							})
						})
				}
				...
				if ( binaryen['BlockId'] === funcBody.id && '' === funcBody.name )
				{
					...
					else
					{
						if ( binaryen['none'] !== funcInfo['results'] )
						{
							...
							c[c.length] = decodedModule.return(d[funcInfo['results']].const(0));
						}
					}
				}
						...
						definitions: arr.map(
							(i,idx) => {
								...
								var walker = function x (ptr) {
									...
									else if ( binaryen['GlobalGetId'] === expr['id'] ||
								...
								return asmJsConstructVariable[i](
									...
									true === output['optimize_for_js'] && (function () {
				...
				body = body.concat(
					(function x (parentExpr, exprPtr) {
						const expr = Object.assign(
							binaryen.getExpressionInfo( exprPtr ), {'srcPtr': exprPtr}
						);
						if ( undefined === expressionList[expr.id] )
							throw [
								'Missing ', expr.id, ' data.'
							].join('');
						...
						if ( parentExpr.length <= parentNodes.length && parentExpr.every(
							(i, idx) => i === parentNodes[parentNodes.length - parentExpr.length + idx]) ) {
							return f[expr.id].apply(null, args);
						}
						else {
							Array.prototype.splice.apply(
								parentNodes, [parentNodes.length, 0].concat(parentExpr)
							);
